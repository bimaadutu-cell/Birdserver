...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.update(servers).set({ suspended: false, updatedAt: new Date() }).where(eq(servers.id, id));
  await db.insert(serverLogs).values({ serverId: id, level: "info", message: `[BirdServer] Server unsuspended by ${session.user.username}.` });
