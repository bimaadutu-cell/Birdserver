...
import { stopServer } from "@/lib/process-manager";

export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const auth = await authenticate(req, "servers:stop");
  ...
  await db.update(servers).set({ status: "STOPPING", updatedAt: new Date() }).where(eq(servers.id, id));
  await stopServer(id, "SIGTERM");
  ...
  return apiSuccess({ status: "STOPPING" });
}
