...
import { startServer, isRunning } from "@/lib/process-manager";

export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const auth = await authenticate(req, "servers:start");
  ...
  if (isRunning(id)) {
    return apiSuccess({ status: "RUNNING", message: "Server is already running." });
  }
  ...
  try {
    ...
    return apiError("START_FAILED", (err as Error).message, 500);
  }
}
